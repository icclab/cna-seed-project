        ...
        public function evaluateBeforeSave(RedBeanModel $model, $attribute)
        {
            ...
            switch($this->trigger->getOperator())
            {
                ...
                case OperatorRules::TYPE_ONE_OF:
                    ...
                    if (in_array(static::sanitize($model->{$attribute}), static::sanitize($this->trigger->value)))
                    {
                        return true;
                    }
                    break;
                ...
                case OperatorRules::TYPE_BECOMES_ONE_OF:
                    ...
                    if (array_key_exists($attribute, $model->originalAttributeValues) &&
                        in_array(static::sanitize($model->{$attribute}), static::sanitize($this->trigger->value)))
                    {
                        return true;
                    }
                    break;
                case OperatorRules::TYPE_WAS_ONE_OF:
                    ...
                    if (array_key_exists($attribute, $model->originalAttributeValues) &&
                        in_array(static::sanitize($model->originalAttributeValues[$attribute]),
                            static::sanitize($this->trigger->value)))
                    {
                        return true;
                    }
