<?php
    ...
    class ArchivedEmailMatchingUtil
    {
        ...
        {
            if ($emailMessage->sender->id > 0 && $emailMessage->sender->personsOrAccounts->count() == 0)
            {
        ...
        {
            if ($emailMessage->sender->id > 0 && $emailMessage->sender->personsOrAccounts->count() > 0 &&
                $emailMessage->sender->personsOrAccounts[0]->isSame($contact))
            {
                foreach ($emailMessage->sender->personsOrAccounts as $personOrAccount)
                {
        ...
        public static function resolveContactToSenderOrRecipient(EmailMessage $emailMessage, $contact)
        {
            if ($emailMessage->sender->id > 0 && $emailMessage->sender->personsOrAccounts->count() == 0)
            {
                 $emailMessage->sender->personsOrAccounts->add($contact);
                 return;
            }
            elseif ($emailMessage->recipients->count() > 0)
            {
                foreach ($emailMessage->recipients as $key => $recipient)
                {
                    if ($recipient->personsOrAccounts->count() == 0)
                    {
                        $emailMessage->recipients->offsetGet($key)->personsOrAccounts->add($contact);
                        return;
                    }
                }
            }
