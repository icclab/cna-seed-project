<?php
    ...
    /**
     * Override the getSelectableRelationsData method so that only non hasMany variants are rendered
     */
    class MergeTagsModelRelationsAndAttributesToRowsAndColumnsReportAdapter extends ModelRelationsAndAttributesToRowsAndColumnsReportAdapter
    {
        /**
        ...
        public function getSelectableRelationsData(RedBeanModel $precedingModel = null, $precedingRelation = null)
        {
            ...
            foreach ($this->model->getAttributes() as $attribute => $notUsed)
            {
                if ($this->model->isRelation($attribute) &&
                    ...
                    $this->model->isRelationTypeAHasOneVariant($attribute)
                    )
                {
