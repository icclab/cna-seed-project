<?php
    ...
    $theme            = 'default';
    $connectionString = 'mysql:host={{getv "/services/mysql/host"}};port={{getv "/services/mysql/port"}};dbname=zurmo'; // Not Coding Standard
    $username         = 'zurmo';
    $password         = 'zurmo';
    $memcacheServers  = array( // An empty array means memcache is not used.
                            {{range $dir := lsdir "/services/memcache"}}
                              {{$hostpath := printf "/services/memcache/%s/host" $dir}}
                              {{$portpath := printf "/services/memcache/%s/port" $dir}}
			      array(
				  'host'   => '{{getv $hostpath}}',
				  'port'   => {{getv $portpath}}, 
				  'weight' => 100,
