            ...
            if (!empty($object->$attribute) && @class_exists($this->modelClassName))
            {
                ...
                if ($mergeTagCount && !$mergeTagsUtil->resolveMergeTagsArrayToAttributes($model, $invalidTags, null, false, $params))
                {
                    ...
                    if (!empty($invalidTags))
                    {
                        ...
                        {
                            $label = EmailTemplateHtmlAndTextContentElement::renderModelAttributeLabel($attribute);
                            $this->addError($object, $attribute, Zurmo::t('EmailTemplatesModule', '{label}: Invalid MergeTag({mergeTag}) used.',
                                                                          array('{mergeTag}' => $tag, '{label}' => $label)));
                        }
                    }
                    else
                    {
